...
/*uint32_t absTime;
uint32_t fftTime;
uint32_t removeTrendTime;
uint32_t ar1Time;
uint32_t varianceTime;
...
extern int16_t data[DATA_COUNT * 3];    // Массив с ускорениями
...
int main()
{
  ...
  for(int part = 0; part < DATA_COUNT * 3; part+=WTF_SIZE * 3)
  {
    for(int i = 0; i<MAX_BUFFER_SIZE * 3; i+=3)
    {
      int16_t x = data[part + i];
      int16_t y = data[part + i + 1];
      int16_t z = data[part + i + 2];
